<?php namespace Edutalk\Base\AssetsManagement;
...
class Assets
{
    ...
    public function __construct()
    {
        if (config('app.env') == 'production') {
            $version = config('edutalk.version', 2.0);
        } else {
            $version = time();
        }
        $this->withVersion = config('edutalk-assets.with_version');
    ...
    public function getAssetsFrom($environment = 'admin')
    {
        ...
        $this->js = array_merge(config('edutalk-assets.default.' . $environment . '.js'), $this->js);
        $this->css = array_merge(config('edutalk-assets.default.' . $environment . '.css'), $this->css);
        $this->fonts = array_merge(config('edutalk-assets.default.' . $environment . '.fonts'), $this->fonts);
    ...
    public function renderStylesheets()
    {
        ...
        foreach ($this->fonts as $row) {
            foreach ($this->getResourceUrls(array_get(config('edutalk-assets.resources.fonts', []), $row)) as $resource) {
                $data .= PHP_EOL . '<link rel="stylesheet" type="text/css" href="' . $resource . $version . '"/>';
            }
        }
        foreach ($this->css as $row) {
            foreach ($this->getResourceUrls(array_get(config('edutalk-assets.resources.css', []), $row)) as $resource) {
                $data .= PHP_EOL . '<link rel="stylesheet" type="text/css" href="' . $resource . $version . '"/>';
            }
        }
    ...
    public function renderScripts($location)
    {
        ...
        foreach ($this->js as $row) {
            foreach ($this->getResourceUrls(array_get(config('edutalk-assets.resources.js', []), $row), $location) as $resource) {
    ...
    {
        $isUseCdn = (config('edutalk-assets.always_use_local')) ? false : (array_get($resources, 'use_cdn', false));
        if ($isUseCdn) {
